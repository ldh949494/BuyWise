...
4. 启动 MySQL。可以复用 Docker Compose 里的 MySQL：
...
5. 启动本机开发后端：
   ...
   该脚本会加载 UTF-8 设置、执行数据库迁移并启动 Uvicorn。如果 `.env` 中是 `MYSQL_HOST=mysql`，脚本会在当前本机进程中改用 `127.0.0.1`，用于连接 Docker 暴露到宿主机的 MySQL。

6. 打开接口文档和健康检查：
   - Swagger UI: `http://127.0.0.1:8000/docs`
   - Health: `http://127.0.0.1:8000/api/v1/health`

也可以直接启动 Uvicorn，但需要确保 `.env` 中的 `MYSQL_HOST` 能被本机解析：
...
导入脚本默认读取 `data/beta-catalog.csv`，按 `sku` upsert，因此可以重复运行。商品图片应先写成可公网访问的真实 URL，随后用 COS 迁移脚本固化为稳定资源。向量索引默认写入 `CHROMA_PERSIST_DIR`。

演示闭环建议使用确定性 demo 种子数据，便于固定提问、推荐命中和商品卡片展示：
...
Swagger 备用演示路径见 `docs/reference/api.md`。后端启动后可运行：
...
如果只做离线 smoke，未配置真实 LLM key，可以使用：
...
4. Android 真机访问电脑上的后端时，不能使用 `10.0.2.2`，需要改成电脑的局域网 IP，例如：
...
全 Docker 模式启动后端和 MySQL：

```powershell
docker compose up --build
```

容器启动后执行迁移和导入。这个模式下 backend 在 Compose 网络内运行，`.env` 中的 `MYSQL_HOST=mysql` 是正确的：
