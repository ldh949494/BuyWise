...
def test_dev_admin_login_allows_builtin_default_when_no_admin_exists() -> None:
    settings.app_env = "dev"
    client = make_client_without_admin()

    response = client.post(
        "/api/v1/admin/auth/login",
        json={"username": "admin", "password": "buywise-admin"},
    )
...
def test_prod_admin_login_rejects_builtin_default() -> None:
...
def test_builtin_default_does_not_override_existing_admin() -> None:
    settings.app_env = "dev"
    client = make_client()
...
def configure_valid_prod_settings() -> None:
