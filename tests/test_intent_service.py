...
@pytest.mark.anyio
async def test_extract_desktop_bundle_recommendation_need_from_text() -> None:
    service = IntentService()

    need = await service.extract("帮我配齐一套6000元以内的桌面装备，包括电脑外设和显示器")

    assert need.intent == "bundle_recommend"
    assert need.budget_max == 6000
    assert need.scenario == "桌面"
    assert need.retrieval_strategy == "bundle"
    assert need.need_clarify is False
