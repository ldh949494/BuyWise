...
def test_import_products_inserts_rows_and_skips_duplicate_names() -> None:
    session_factory = make_session_factory()

    first_result = import_products(session_factory=session_factory)
    second_result = import_products(session_factory=session_factory)

    with session_factory() as db:
        products = db.query(Product).order_by(Product.id).all()

    assert first_result == {"inserted": 50, "skipped": 0}
    assert second_result == {"inserted": 0, "skipped": 50}
