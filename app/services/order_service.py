...
class OrderService:
    ...
    def _build_order_and_item(
        ...
        feedback_due_at = self._initial_feedback_due_at(self._is_external_purchase(payload), now)
        order = self._build_order(payload, user_ref, product, feedback_due_at, now)
        item = self._build_order_item(payload, product, feedback_due_at, now)
        return order, item
    ...
    def _initial_feedback_due_at(self, external_purchase: bool, now: datetime) -> datetime | None:
        if not external_purchase:
            return None
        if settings.external_purchase_feedback_mode == "immediate":
            return now
        return now + timedelta(days=settings.feedback_delay_days)
