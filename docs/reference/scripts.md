...
- `scripts/validate_secrets.py`：扫描 Git 跟踪文件中的高置信度密钥形态，只输出文件和行号，不输出疑似密钥值。
...
- `app.scripts.import_products`：从 CSV 导入商品，默认读取本地忽略文件 `.\data\beta-catalog.csv`。CSV 必须包含 `sku`、`name`、`category`、`price`、`tags`；脚本先整批校验，再按 `sku` upsert，输出 `inserted`、`updated`、`failed`。Closed beta 真实目录使用 `--require-real-assets`，要求每行包含真实 `product_url`、真实图片 URL，以及 `stock` 或 `stock_status`。可传入 `--artifact-json <path>` 留存统一 job artifact。
...
- `app.scripts.migrate_product_images_to_cos`：把商品表中现有 `image_url` / `image_urls` 指向的图片下载后上传到腾讯 COS，并把商品记录更新为 COS URL。默认 dry-run，不写数据库；确认后运行 `python -m app.scripts.migrate_product_images_to_cos --apply`。可传入 `--artifact-json <path>` 留存统一 job artifact；失败时 artifact 会包含失败 URL、SKU 和异常摘要。依赖 `.env` 中的 `UPLOAD_PROVIDER=cos`、Tencent Secret、`COS_BUCKET` 和 `COS_REGION`。
- `app.scripts.validate_product_images`：校验商品图片契约。默认校验 `data/products.csv` 和 seed profiles，要求图片 URL 使用 COS host、`image_urls` 包含主图且不使用占位域名。发布前对实际导入的单个目录 CSV 运行唯一性校验，例如 `python -m app.scripts.validate_product_images --csv .\data\products.csv --skip-seed-profiles --require-unique-image-urls --check-http --min-image-bytes 10000 --require-unique-http-etags`；如果同时维护 `products.csv` 和 `beta-catalog.csv` 两份镜像目录，应分别运行，避免同一批商品跨文件互相判重。
- `app.scripts.rewrite_product_images`：从维护好的 CSV 按 `sku` 重写数据库商品的 `image_url` 和 `image_urls`。默认 dry-run；确认后运行 `python -m app.scripts.rewrite_product_images --source .\data\products.csv --apply`。适合 CSV 图片已经修正，但数据库中历史商品仍指向旧图或 403 图的场景。
- `app.scripts.repair_product_image_acls`：修复 CSV 和 seed 引用的 COS `product-images/` 对象 ACL，确保对象为 `public-read`。默认 dry-run；确认后运行 `python -m app.scripts.repair_product_image_acls --apply`。新上传逻辑已经默认写 `public-read`，该脚本用于修复历史对象或导入前门禁。
