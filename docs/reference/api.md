...
RAG 检索质量通过 `data/rag_eval/` 下的 profile 评测集跟踪。运行 `python -m app.scripts.evaluate_rag --profile android-contract` 可验证 Android 合同商品，运行 `python -m app.scripts.evaluate_rag --profile demo` 可验证演示商品，运行 `python -m app.scripts.evaluate_rag --profile beta-fixture --retrieval vector` 可用 beta fixture 重建临时 Chroma 索引并评估真实召回链路。报告包含 `recall@k`、`top1_accuracy`、`mrr@k`、当前失败案例和 case-level diagnostics。`tests/test_rag_eval.py` 对固定评测集提供轻量回归门禁。运行 `python -m app.scripts.check_vector_index --profile demo` 可检查演示索引缺失或陈旧商品 ID。
