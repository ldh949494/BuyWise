...
`POST /api/v1/ai/chat/stream`、`POST /api/v1/ai/guide/stream` 和 `POST /api/v1/ai/guide/follow-up/stream` 返回 `text/event-stream`。事件类型固定为：
...
然后从 `android-app/` 构建或运行 Android 应用。应用应能在启动时加载商品、自动对比可用商品、从后端刷新商品详情，并通过 `/api/v1/ai/guide/stream` 开始导购、通过 `/api/v1/ai/guide/follow-up/stream` 流式展示追问回复。
