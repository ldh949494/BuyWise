...
当 `APP_ENV=prod` 时，应用创建阶段会执行生产配置校验。生产环境必须关闭 debug，配置非占位 API key、readiness token、admin JWT secret、user JWT secret、MySQL 密码和 secret，关闭 OTP mock，并使用显式 CORS origin。启用 credentials 时不得使用通配 CORS origin。`UPLOAD_PROVIDER=cos` 时必须配置 Tencent Secret、COS bucket 和 region。
