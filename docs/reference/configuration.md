...
Closed beta 阶段默认 `SPEECH_PROVIDER=mock`，语音不作为发布门禁。`APP_ENV=prod` 且 `ALLOW_MOCK_PROVIDERS_IN_PROD=false` 时，`LLM_PROVIDER` 和 `VISION_PROVIDER` 不能为 `mock`，但 `SPEECH_PROVIDER=mock` 允许。
...
当 `APP_ENV=prod` 时，应用创建阶段会执行生产配置校验。生产环境必须关闭 debug，配置非占位 API key、readiness token、MySQL 密码和 secret，并使用显式 CORS origin。启用 credentials 时不得使用通配 CORS origin。`UPLOAD_PROVIDER=cos` 时必须配置 Tencent Secret、COS bucket 和 region。
