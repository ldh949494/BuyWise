...
- `AUTH_OTP_MOCK_ENABLED`
- `AUTH_OTP_EXPIRE_MINUTES`
- `AUTH_OTP_COOLDOWN_SECONDS`
- `AUTH_OTP_MAX_ATTEMPTS`
- `AUTH_OTP_PHONE_HOUR_LIMIT`
- `AUTH_OTP_PHONE_DAY_LIMIT`
- `AUTH_OTP_IP_MINUTE_LIMIT`
- `AUTH_OTP_IP_HOUR_LIMIT`
...
- `CHAT_STREAM_MAX_SECONDS`
- `CHAT_STREAM_FAST_PRODUCTS_ENABLED`
- `CHAT_STREAM_FAST_PRODUCTS_LIMIT`
- `CHAT_STREAM_FAST_REPLY_MAX_TOKENS`
- `CHAT_RATE_LIMIT_PER_MINUTE`
- `CHAT_AUTH_RATE_LIMIT_PER_MINUTE`
- `CHAT_ANON_RATE_LIMIT_PER_MINUTE`
- `CHAT_SESSION_RATE_LIMIT_PER_MINUTE`
- `CHAT_SESSION_TOKENS_ENABLED`
- `CHAT_ANON_SESSION_TTL_HOURS`
- `CHAT_CONTEXT_MAX_MESSAGES`
- `CHAT_CONTEXT_MAX_CHARS`
- `AI_CHAT_ACTIONS_REQUIRE_AUTH_IN_PROD`
- `AI_CHECKOUT_CONFIRMATION_REQUIRED`
- `AI_MEDIA_URL_ALLOWLIST_ENABLED`
- `AI_MEDIA_ALLOWED_HOSTS`
...
Closed beta 阶段可临时启用 mock provider 恢复演示链路。`APP_ENV=prod` 且 `ALLOW_MOCK_PROVIDERS_IN_PROD=false` 时，`LLM_PROVIDER`、`VISION_PROVIDER` 和 `EMBEDDING_PROVIDER` 不能为 `mock`，但 `SPEECH_PROVIDER=mock` 允许。显式设置 `ALLOW_MOCK_PROVIDERS_IN_PROD=true` 时，也允许 closed beta 使用 mock OTP、mock vision 和 mock speech；正式生产不得开启该开关。

`UPLOAD_PROVIDER` 支持 `local` 或 `cos`。本地上传保存到 `UPLOAD_DIR`；COS 上传使用 `TENCENT_SECRET_ID`、`TENCENT_SECRET_KEY`、`COS_BUCKET` 和 `COS_REGION`。

`AI_LLM_MAX_CONCURRENCY`、`AI_VISION_MAX_CONCURRENCY` 和 `AI_SPEECH_MAX_CONCURRENCY` 控制 closed beta 阶段进程内外部 AI provider 并发闸门。`AI_PROVIDER_TIMEOUT_SECONDS` 控制外部 provider 调用超时，`CAPACITY_RETRY_AFTER_SECONDS` 控制容量不足时返回的 `Retry-After`。`CHAT_STREAM_HEARTBEAT_SECONDS` 控制 `/ai/chat/stream` 空闲多久发送一次 SSE `heartbeat`，`CHAT_STREAM_MAX_SECONDS` 控制单次流式响应的最长持续时间，超时后发送 `error.code=chat_stream_timeout`。`CHAT_STREAM_FAST_PRODUCTS_ENABLED` 控制文本导购 stream 是否在无图片、无语音、无历史上下文且规则可识别品类时先走快速 DB 候选；`CHAT_STREAM_FAST_PRODUCTS_LIMIT` 控制首屏候选数量；`CHAT_STREAM_FAST_REPLY_MAX_TOKENS` 控制快速路径下最终导购短回复的生成上限。`CHAT_RATE_LIMIT_PER_MINUTE` 控制默认 AI 聊天入口限流；`CHAT_AUTH_RATE_LIMIT_PER_MINUTE`、`CHAT_ANON_RATE_LIMIT_PER_MINUTE` 和 `CHAT_SESSION_RATE_LIMIT_PER_MINUTE` 可分别覆盖登录、匿名和单会话 AI 限流，值为 `0` 时回退到默认值。`CHAT_SESSION_TOKENS_ENABLED=true` 时，匿名会话首次响应会返回 `session_token`，续聊必须带回该 token；生产环境必须启用。`CHAT_ANON_SESSION_TTL_HOURS` 控制匿名会话 token 有效期，`CHAT_CONTEXT_MAX_MESSAGES` 和 `CHAT_CONTEXT_MAX_CHARS` 控制对话上下文窗口。`AI_CHAT_ACTIONS_REQUIRE_AUTH_IN_PROD` 和 `AI_CHECKOUT_CONFIRMATION_REQUIRED` 控制 prod 下聊天动作的登录和确认要求。`AI_MEDIA_URL_ALLOWLIST_ENABLED` 和 `AI_MEDIA_ALLOWED_HOSTS` 控制图片/音频 URL 允许列表，生产环境必须启用。当前限流和容量保护只在单进程内生效。
...
`USER_JWT_SECRET` 用于普通用户手机号验证码登录后签发 user access JWT，必须与 `ADMIN_JWT_SECRET` 分离。`USER_JWT_EXPIRE_MINUTES` 默认 15 分钟，`USER_REFRESH_TOKEN_EXPIRE_DAYS` 默认 30 天。普通用户 refresh token 只以哈希形式存储在数据库中，refresh 成功会轮换 token。

`AUTH_OTP_MOCK_ENABLED` 控制普通用户 OTP mock provider。dev/test 默认开启并返回 `debug_otp`，当前固定为 `123456`；prod 默认必须关闭 mock OTP，且在未接入真实短信 provider 前 OTP 请求会返回 `otp_provider_not_configured`。closed beta 如需固定验证码演示，可同时设置 `ALLOW_MOCK_PROVIDERS_IN_PROD=true` 和 `AUTH_OTP_MOCK_ENABLED=true`；prod 响应仍不返回 `debug_otp`，测试人员直接输入 `123456`。`AUTH_OTP_EXPIRE_MINUTES`、`AUTH_OTP_COOLDOWN_SECONDS`、`AUTH_OTP_MAX_ATTEMPTS`、`AUTH_OTP_PHONE_HOUR_LIMIT`、`AUTH_OTP_PHONE_DAY_LIMIT`、`AUTH_OTP_IP_MINUTE_LIMIT` 和 `AUTH_OTP_IP_HOUR_LIMIT` 控制验证码过期、冷却、尝试次数和基础防刷限制。
...
当 `APP_ENV=prod` 时，应用创建阶段会执行生产配置校验。生产环境必须关闭 debug，配置非占位 API key、readiness token、admin JWT secret、user JWT secret、MySQL 密码和 secret，默认关闭 OTP mock，并使用显式 CORS origin。启用 credentials 时不得使用通配 CORS origin。`UPLOAD_PROVIDER=cos` 时必须配置 Tencent Secret、COS bucket 和 region。
