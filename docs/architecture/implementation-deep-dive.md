...
5. 后端先以 `products` SSE 事件返回商品卡片，再以 `token` 事件流式返回 LLM 文案，最后发送 `done`。
6. Android `GuideViewModel` 把 SSE 事件转换为 Compose 可观察状态，页面增量展示回复和推荐卡片。
...
- `products` 更新推荐商品，并在聊天模式下挂到当前 assistant 消息。
- `done` 用完整 reply 补偿 token 拼接结果，并结束 streaming 状态。
- `error` 设置错误信息并结束 streaming。
- `heartbeat` 不改变 UI。

这一设计使后端可以先返回商品卡片，再慢慢生成自然语言解释；Android UI 不需要等 LLM 完整结束才展示推荐。
