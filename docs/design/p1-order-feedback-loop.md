...
已购反馈复用并扩展现有 `reviews` 表，不新建平行 feedback 表。`reviews` 作为统一口碑事实表，新增 `order_item_id`、`user_ref`、`source`、`verified_purchase`、`purchase_evidence`、`rating`、`usage_context`、`pros_tags`、`cons_tags`、`met_expectation`、`status`、`submitted_at`、`updated_at` 等字段。导入评论继续作为 `source=imported`，收货后反馈作为 `source=buywise_post_delivery`。Closed beta 外部购买记录使用 `purchase_evidence=buywise_recorded`，不声明平台验真；未来平台验真订单才使用 `platform_verified`。每个订单项最多一条 active 反馈；用户可更新反馈，也可撤回为 `status=withdrawn`，分析只纳入 active 反馈。
...
订单和评价先使用轻量 `user_ref`，不在本阶段建设完整账号登录系统。API 通过 provider 获取当前 subject；如果当前没有真实 auth provider，则使用 demo/default subject。创建已购反馈时必须校验订单项属于当前 `user_ref` 且已经收货，购买证据等级只能由服务端根据订单项和后续验真能力判定，客户端不能伪造。
...
- 服务测试覆盖非缺货校验、订单项归属校验、已收货校验、`purchase_evidence` 服务端判定、重复 active 反馈限制和反馈权重公式。
...
- Android 已实现商品详情记录购买、首页待评价提示、内联反馈表单、优缺点标签、是否符合预期、缺 token 禁用提示和商品详情评价摘要。
- 剩余边界是完整订单中心、真实支付/履约、平台订单验真和长期个人化反馈建模；这些不属于 P1 实现范围。
