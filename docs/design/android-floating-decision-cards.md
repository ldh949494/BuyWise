...
- 只展示能帮助用户完成下一步购物决策的字段。
- 默认展示商品名、价格、评分、一个推荐理由、对比状态和关键操作。
- 条件展示异常库存、真实识别结果、真实用户需求摘要、真实购买反馈和明确风险。
- 默认隐藏开发信息、模拟平台、模拟销量、重复匹配分、固定示例标签、空模块、泛化说明和排序机制细节。
- 商品详情聚焦购买结论、为什么推荐和购买前注意；对比页只保留少量关键维度和强结论。
