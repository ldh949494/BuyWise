...
场景化组合推荐在 BuyWise 中按“单轮编排出的组合方案”验收，不要求新增独立的购物车或真实结算能力。

- 入口接口以 `POST /api/v1/ai/chat` 和 `POST /api/v1/ai/chat/stream` 为准，`structured_need.intent` 需要能稳定表达组合型场景，例如“场景化组合推荐”或等价语义。
- 响应至少包含 `reply`、`structured_need` 和 `products`；其中 `products` 需要能承载跨类目的组合候选，而不是只返回单一商品。
- 组合方案必须同时满足预算、类目、场景、偏好和排除条件，不允许为了拼出组合而突破用户显式约束。
- 当信息不足时，系统先追问关键缺口，再输出组合方案；当组合编排失败时，退化为单品推荐或明确追问，不返回空白结果。
- 验收测试至少覆盖以下场景：单轮输入触发组合推荐、多类目候选检索、否定条件排除、预算约束保持、SSE 与 JSON 返回一致。

### 购物车与下单范围

BuyWise 当前纳入的是“记录购买 + 订单影子模型 + 待评价 + 已购反馈”闭环，不纳入完整购物车和真实下单系统。

- 纳入范围：记录购买、模拟订单状态推进、待评价提示、已购评价提交/更新/撤回。
- 不纳入范围：购物车 CRUD、数量编辑、收货地址管理、支付、真实 checkout、退款和售后。
- API 与 Android 文案应继续使用“记录购买”“已购反馈”“购买后反馈”等表述，不把现有影子订单误写成真实交易。
- 如果未来需要购物车或真实下单，应单独立项，不复用当前 closed beta 验收口径。
