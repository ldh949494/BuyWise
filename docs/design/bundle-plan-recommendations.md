...
- 已实现 `bundle_recommend` 意图识别、`ChatResponse.bundle_plans` schema、方案生成服务、JSON chat 和 SSE chat 返回、demo 桌面组合商品、Android 强类型解析、方案卡片和聊天内方案展示。
- 第一版仍不包含一键跨平台下单、复杂预算手动编辑、公开分享链接、多人协作、长期价格追踪或自动重算历史方案。
